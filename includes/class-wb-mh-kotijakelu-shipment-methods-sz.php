...
/**
 * WB_Mh_Kotijakelu_Shipping_Method_sz Class
 *
 * @class WB_Mh_Kotijakelu_Shipping_Method_sz
 * @version	1.0.0
 * @since 1.0.0
 ...
 * @access public
 */
function WB_Mh_Kotijakelu_Shipping_Method_sz_Init() {

	if ( ! class_exists( 'WB_Mh_Kotijakelu_Shipping_Method_sz' ) ) {

		class WB_Mh_Kotijakelu_Shipping_Method_sz extends WC_Shipping_Method {
			...
			*/
			public function __construct( $instance_id = 0 ) {
				$this->id = 'wb_mh_kotijakelu_shipping_method_sz'; // Id for your shipping method. Should be uunique.
				$this->instance_id = absint( $instance_id );
				$this->method_title = __( 'Matkahuolto Kotijakelu (Matkahuolto toimitustavat)', 'wb-matkahuolto-toimitustavat' ); // Title shown in admin
				$this->method_description = __( 'Matkahuolto kotijakelu', 'wb-matkahuolto-toimitustavat' ); // Description shown in admin
				$this->supports = array(
					'shipping-zones',
					'instance-settings',
				);
			...
			*/
			function init() {
				...
				$this->kotijakelu_paino0		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_paino0') ));
				$this->kotijakelu_hinta0		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_hinta0') ));

				$this->kotijakelu_paino1		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_paino1') ));
				$this->kotijakelu_hinta1		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_hinta1') ));

				$this->kotijakelu_paino2		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_paino2') ));
				$this->kotijakelu_hinta2		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_hinta2') ));

				$this->kotijakelu_paino3		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_paino3') ));
				$this->kotijakelu_hinta3		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_hinta3') ));

				$this->kotijakelu_paino4		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_paino4') ));
				$this->kotijakelu_hinta4		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_hinta4') ));

				$this->kotijakelu_paino5		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_paino5') ));
				$this->kotijakelu_hinta5		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_hinta5') ));

				$this->kotijakelu_paino6		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_paino6') ));
				$this->kotijakelu_hinta6		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_hinta6') ));

				$this->kotijakelu_paino7		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_paino7') ));
				$this->kotijakelu_hinta7		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_hinta7') ));

				$this->kotijakelu_paino8		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_paino8') ));
				$this->kotijakelu_hinta8		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_hinta8') ));

				$this->kotijakelu_paino9		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_paino9') ));
				$this->kotijakelu_hinta9		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_hinta9') ));

				$this->kotijakelu_paino10		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_paino10') ));
				$this->kotijakelu_hinta10		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_hinta10') ));

				$this->kotijakelu_paino11		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_paino11') ));
				$this->kotijakelu_hinta11		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_hinta11') ));

				$this->kotijakelu_paino12		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_paino12') ));
				$this->kotijakelu_hinta12		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_hinta12') ));

				$this->kotijakelu_paino13		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_paino13') ));
				$this->kotijakelu_hinta13		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_hinta13') ));

				$this->kotijakelu_paino14		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_paino14') ));
				$this->kotijakelu_hinta14		   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_hinta14') ));

				$this->kotijakelu_ilm_toim	   	   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_ilm_toim') ));
				$this->kotijakelu_kas_kulut	   = str_replace(",", ".", esc_attr( $this->get_option('kotijakelu_kas_kulut') ));

				$this->kj_hinta_yli	   		       = str_replace(",", ".", esc_attr( $this->get_option('kj_hinta_yli') ));

				$this->kj_max_korkeus     		   = str_replace(",", ".", esc_attr( $this->get_option('kj_max_korkeus') ));
				$this->kj_max_pituus      		   = str_replace(",", ".", esc_attr( $this->get_option('kj_max_pituus') ));
				$this->kj_max_leveys      		   = str_replace(",", ".", esc_attr( $this->get_option('kj_max_leveys') ));
				$this->kj_max_paino      		   = str_replace(",", ".", esc_attr( $this->get_option('kj_max_paino') ));

				$this->kj_max_paino_select     	   = esc_attr( $this->get_option('kj_max_paino_select'));

				$this->kj_kuponki			   	   = esc_attr( $this->get_option('kj_kuponki') );
				$this->kj_kuponki_kaikki		   = esc_attr( $this->get_option('kj_kuponki_kaikki') );
			...
			function init_form_fields() {
				...
				$this->instance_form_fields = array(
					...
					'kotijakelu_ilm_toim'	   => array(
						'title'            => __('kotijakelu ilmaisen toimituksen raja', 'wb-matkahuolto-toimitustavat'),
						'type'             => 'text',
						'placeholder'	   => '100',
			...
			*/
			public function calculate_shipping( $package = array() ) {
				...
				$paino0 = $this->kotijakelu_paino0;
				$paino1 = $this->kotijakelu_paino1;
				$paino2 = $this->kotijakelu_paino2;
				$paino3 = $this->kotijakelu_paino3;
				$paino4 = $this->kotijakelu_paino4;
				$paino5 = $this->kotijakelu_paino5;
				$paino6 = $this->kotijakelu_paino6;
				$paino7 = $this->kotijakelu_paino7;
				$paino8 = $this->kotijakelu_paino8;
				$paino9 = $this->kotijakelu_paino9;
				$paino10 = $this->kotijakelu_paino10;
				$paino11 = $this->kotijakelu_paino11;
				$paino12 = $this->kotijakelu_paino12;
				$paino13 = $this->kotijakelu_paino13;
				$paino14 = $this->kotijakelu_paino14;

				$kotijakelu_hinta_yli = $this->kj_hinta_yli;

				$ilm_toim = floatval($this->kotijakelu_ilm_toim);

				// Kuponki
				$has_coupon = false;
				$all_coupons = array();
				$annettu_koodi = $this->kj_kuponki;
				$annettu_koodi_array_or_not = false;

				$salli_kaikki_kupongit = $this->kj_kuponki_kaikki;
				...
				if ( ($ilm_toim !='') && ($ilm_toim <= floatval($cart_price)) OR $has_coupon ) {
					...
					if ( (0 <= $weight) && ($weight <= $paino0) ) {
						$lopullinen_hinta = $this->kotijakelu_hinta0 + $this->kotijakelu_kas_kulut;
					} elseif ( ($paino0 <= $weight) && ($weight <= $paino1) ) {
						$lopullinen_hinta = $this->kotijakelu_hinta1 + $this->kotijakelu_kas_kulut;
					} elseif ( ($paino1 <= $weight) && ($weight <= $paino2) ) {
						$lopullinen_hinta = $this->kotijakelu_hinta2 + $this->kotijakelu_kas_kulut;
					} elseif ( ($paino2 <= $weight) && ($weight <= $paino3) ) {
						$lopullinen_hinta = $this->kotijakelu_hinta3 + $this->kotijakelu_kas_kulut;
					} elseif ( ($paino3 <= $weight) && ($weight <= $paino4) ) {
						$lopullinen_hinta = $this->kotijakelu_hinta4 + $this->kotijakelu_kas_kulut;
					} elseif ( ($paino4 <= $weight) && ($weight <= $paino5) ) {
						$lopullinen_hinta = $this->kotijakelu_hinta5 + $this->kotijakelu_kas_kulut;
					} elseif ( ($paino5 <= $weight) && ($weight <= $paino6) ) {
						$lopullinen_hinta = $this->kotijakelu_hinta6 + $this->kotijakelu_kas_kulut;
					} elseif ( ($paino6 <= $weight) && ($weight <= $paino7) ) {
						$lopullinen_hinta = $this->kotijakelu_hinta7 + $this->kotijakelu_kas_kulut;
					} elseif ( ($paino7 <= $weight) && ($weight <= $paino8) ) {
						$lopullinen_hinta = $this->kotijakelu_hinta8 + $this->kotijakelu_kas_kulut;
					} elseif ( ($paino8 <= $weight) && ($weight <= $paino9) ) {
						$lopullinen_hinta = $this->kotijakelu_hinta9 + $this->kotijakelu_kas_kulut;
					} elseif ( ($paino9 <= $weight) && ($weight <= $paino10) ) {
						$lopullinen_hinta = $this->kotijakelu_hinta10 + $this->kotijakelu_kas_kulut;
					} elseif ( ($paino10 <= $weight) && ($weight <= $paino11) ) {
						$lopullinen_hinta = $this->kotijakelu_hinta11 + $this->kotijakelu_kas_kulut;
					} elseif ( ($paino11 <= $weight) && ($weight <= $paino12) ) {
						$lopullinen_hinta = $this->kotijakelu_hinta12 + $this->kotijakelu_kas_kulut;
					} elseif ( ($paino12 <= $weight) && ($weight <= $paino13) ) {
						$lopullinen_hinta = $this->kotijakelu_hinta13 + $this->kotijakelu_kas_kulut;
					} elseif ( ($paino13 <= $weight) && ($weight <= $paino14) ) {
						$lopullinen_hinta = $this->kotijakelu_hinta14 + $this->kotijakelu_kas_kulut;
					} elseif ( $weight > $paino14 && $kotijakelu_hinta_yli != '') {
						$ylipaino = ($weight - $paino14);
						$ylipaino_kerroin = ceil(($weight - $paino14) / 20 + 1);

						if($ylipaino < 20) {
							$ylihinta = $kotijakelu_hinta_yli;
						} else {
							$ylihinta = ($ylipaino_kerroin) * $kotijakelu_hinta_yli;
						}

						$lopullinen_hinta = $this->kotijakelu_hinta14 + $this->kotijakelu_kas_kulut + $ylihinta;
					} else {
						$lopullinen_hinta = $this->kotijakelu_hinta14 + $this->kotijakelu_kas_kulut;
					}
				}

				$rate = apply_filters('wb_mh_kotijakelu_rate_filter', array(
...
add_action( 'woocommerce_shipping_init', 'WB_Mh_Kotijakelu_Shipping_Method_sz_init' );

function add_WB_Mh_Kotijakelu_Shipping_Method_sz( $methods ) {

	$methods['wb_mh_kotijakelu_shipping_method_sz'] = 'WB_Mh_Kotijakelu_Shipping_Method_sz';
	return $methods;

}

add_filter( 'woocommerce_shipping_methods', 'add_WB_Mh_Kotijakelu_Shipping_Method_sz' );

function hide_show_mh_kj_sz_mh( $rates, $package ) {
	$woocommerce = function_exists('WC') ? WC() : $GLOBALS['woocommerce'];
	$items = $woocommerce->cart->get_cart();
	...
	if(isset($rates)) {
		...
		foreach ($shippingIds as $shipping_id) {
			if (strpos($shipping_id, 'wb_mh_kotijakelu_shipping_method_sz') !== false) {
				$get_the_id = str_replace('wb_mh_kotijakelu_shipping_method_sz', '', $shipping_id);
				$get_the_id = str_replace(':', '', $get_the_id);
			}
		}

		$kassan_tiedot = new cart_items_mh_kj_sz_mh();
		...
		$kj_shipping_method = new WB_Mh_Kotijakelu_Shipping_Method_sz( $instance_id = $get_the_id );

		if(!empty($kj_shipping_method->kj_max_paino_select) && $kj_shipping_method->kj_max_paino_select == 'ei') {
			$cart_weight = 0;
		} else {
			$cart_weight = $woocommerce->cart->cart_contents_weight;
		}	

		if(!empty($kj_shipping_method->kj_max_korkeus)) {
			$max_korkeus = $kj_shipping_method->kj_max_korkeus;
		} else {
			$max_korkeus = 166;
		}

		if(!empty($kj_shipping_method->kj_max_leveys)) {
			$max_leveys = $kj_shipping_method->kj_max_leveys;
		} else {
			$max_leveys = 166;
		}

		if(!empty($kj_shipping_method->kj_max_pituus)) {
			$max_pituus = $kj_shipping_method->kj_max_pituus;
		} else {
			$max_pituus = 166;
		}

		if(!empty($kj_shipping_method->kj_max_paino)) {
			$max_paino = floatval($kj_shipping_method->kj_max_paino);
		} else {
			$max_paino = 55;
		}
		...
		if ( max($height) > $max_korkeus OR max($length) > $max_pituus OR max($width) > $max_leveys OR max($weight) > $max_paino OR $cart_weight > $max_paino ) {
			...
			foreach ( $rates as $rate_id => $rate ) {
				if ( 'wb_mh_kotijakelu_shipping_method_sz' !== $rate->method_id ) {
...
add_filter( 'woocommerce_package_rates', 'hide_show_mh_kj_sz_mh' , 10, 2 );
...
* @return void
*/
function mh_kj_weight_sz_mh( $order_id ) {
	global $woocommerce;
	...
	foreach( $chosen_methods as $shipping_method_used) {
		if(stristr($shipping_method_used, 'wb_mh_kotijakelu_shipping_method_sz')) {
...
add_action('woocommerce_checkout_update_order_meta', 'mh_kj_weight_sz_mh');
